...
export class ArticlesFeedComponent  {
  ...
  private article_ID: any;
  private articles_category: string | undefined;
  public myDate!:Date;
  ...
  var titleValue = title?.value
    ...
    const res = fetch(LoginService.backAddress+"setArticles", {method: "POST", body: JSON.stringify(data), credentials: 'include'});
    res.then(response => { return response.json(); }).then(x => {
      this.afterCreateArticle(x);
    });
  }

  afterCreateArticle(s: JSON) :void{
...
goToCategory(id: any){
  this.articles_category=id;
this.router.navigate(["articles-sorted/"+this.articles_category])
}
  setArticle() {
  var title = document.getElementById("title") as HTMLInputElement
  var titleValue = title?.value
